...
def test_all_connections():
    ...
    print("="*70)
    print("\nArchitecture:")
    print("  Computer A (Neural Network): Google Gemini API (cloud-based)")
    ...
    # Test 1: Neural Network (Google Gemini)
    print("\n" + "-"*70)
    print("[1] NEURAL NETWORK (GOOGLE GEMINI) TEST")
    print("    Cloud-based Google Gemini API")
    print("    Model: " + settings.GEMINI_MODEL)
    print("-"*70)
    
    try:
        llm_health = gemini_service.test_llm_connection()
        ...
        if status == "connected":
            ...
            print(f"✗ Connection failed to Google Gemini")
    ...
    print(f"    Backend connects to Google Gemini API")
    print("-"*70)
    ...
    try:
        ...
        if status == "connected":
            print(f"✓ Backend is connected to Google Gemini")
            results["backend_to_neural_network"] = "success"
        else:
            print(f"✗ Backend cannot reach Google Gemini")
    ...
    if all_success:
        ...
        print("\nCommon fixes:")
        print("  1. Google Gemini: Verify GOOGLE_GEMINI_API_KEY in .env file")
        print("  2. Backend: Ensure running on Computer B: uvicorn app.main:app --reload --host 0.0.0.0")
        print("  3. Check .env file for correct GOOGLE_GEMINI_API_KEY and ALLOWED_ORIGINS")
        print("="*70 + "\n")
        return 1
