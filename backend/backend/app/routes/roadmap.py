...
def generate_roadmap_options(
    ...
    # Generate roadmap options using Google Gemini
    roadmap_options = gemini_service.generate_roadmap_options(
        goal=request.goal,
...
@router.get("/current")
def get_current_roadmap(authorization: str = Header(None), db: Session = Depends(get_db)):
...
def get_next_semester_recommendations(
    ...
    recommendations = gemini_service.recommend_next_semester_courses(
